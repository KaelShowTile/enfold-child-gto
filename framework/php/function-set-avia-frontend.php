		...
		if( 1 != $pages )
		{
			...
			for( $i = 1; $i <= $pages; $i++ )
			{
				if( 1 != $pages &&( ! ( $i >= $paged+$range + 1 || $i <= $paged - $range-1 ) || $pages <= $showitems ) )
				{
					...
					$output .= ( $paged == $i ) ? "<span class='{$class}'>{$i}</span>" : "<a href='" . avia_extended_pagination_link( $method, $i, $query_arg ) . "' {$anchorLink} class='{$class}' >{$i}</a>";
				}
			}
...
if( ! function_exists( 'avia_get_theme_version' ) )
{
	 ...
	 * @since 4.3.2
	 * @added_by Günter
	 * @param string $which			'parent' | 'current'
	 * @return string
	 */
...
if( ! function_exists( 'avia_get_theme_name' ) )
{
	/**
	 * Helper function that returns the (parent) theme name
	 *
	 * @since 6.0
	 * @added_by Günter
	 * @param string $which			'parent' | 'current'
	 * @return string
	 */
	function avia_get_theme_name( $which = 'parent' )
	{
		$theme = wp_get_theme();
		if( false !== $theme->parent() && ( 'parent' == $which ) )
		{
